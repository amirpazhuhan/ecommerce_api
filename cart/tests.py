...
class CartAPITests(APITestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username="cart-user", email="cart-user@example.com", password="safe-password-123"
        )
    ...
    def test_adding_item_accumulates_quantity(self):
        ...
        first = self.client.post("/api/v1/cart/items/", {"product_id": self.product.id, "quantity": 1}, format="json")
        second = self.client.post("/api/v1/cart/items/", {"product_id": self.product.id, "quantity": 2}, format="json")
    ...
    def test_cart_rejects_quantity_above_stock(self):
        ...
        response = self.client.post("/api/v1/cart/items/", {"product_id": self.product.id, "quantity": 4}, format="json")
    ...
    def test_deleting_item_only_affects_current_users_cart(self):
        other_user = get_user_model().objects.create_user(
            username="other-user", email="other@example.com", password="safe-password-123"
        )
        ...
        response = self.client.delete("/api/v1/cart/items/delete/", {"product_id": self.product.id}, format="json")

        self.assertEqual(response.status_code, 204)
        self.assertFalse(Item.objects.filter(pk=own_item.pk).exists())
        self.assertTrue(Item.objects.filter(cart=other_cart, product=self.product).exists())
